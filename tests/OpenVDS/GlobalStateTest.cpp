...
TEST(GlobalState, basic)
{
  auto downloaded = OpenVDS::GetGlobalState()->GetBytesDownloaded(OpenVDS::OpenOptions::InMemory);
  auto downloadedChunkCount = OpenVDS::GetGlobalState()->GetChunksDownloaded(OpenVDS::OpenOptions::InMemory);
  auto decompressed = OpenVDS::GetGlobalState()->GetBytesDecompressed(OpenVDS::OpenOptions::InMemory);
  auto decompressedChunkCount = OpenVDS::GetGlobalState()->GetChunksDecompressed(OpenVDS::OpenOptions::InMemory);

  ASSERT_TRUE(downloaded == 0);
  ASSERT_TRUE(downloadedChunkCount == 0);
  ASSERT_TRUE(decompressed == downloaded);
  ASSERT_TRUE(decompressedChunkCount == 0);
  ...
  for (int thread = 0; thread < threadCount; thread++)
  {
    ...
    auto threadedRequest = [&threadRequestData, accessManager] () mutable
    {
      ...
      for (auto& requestData : threadRequestData)
      {
        ...
        requestData.request = accessManager.RequestVolumeSubset(requestData.bufferFloat.data(), requestData.bufferFloat.size() * sizeof(float), OpenVDS::Dimensions_012, 0, 0, requestData.minPos, requestData.maxPos);
      }
      bool success = true;
      for (auto& requestData : threadRequestData)
      {
  ...
  downloaded = OpenVDS::GetGlobalState()->GetBytesDownloaded(OpenVDS::OpenOptions::InMemory);
  downloadedChunkCount = OpenVDS::GetGlobalState()->GetChunksDownloaded(OpenVDS::OpenOptions::InMemory);
  decompressed = OpenVDS::GetGlobalState()->GetBytesDecompressed(OpenVDS::OpenOptions::InMemory);
  decompressedChunkCount = OpenVDS::GetGlobalState()->GetChunksDecompressed(OpenVDS::OpenOptions::InMemory);
  ASSERT_TRUE(downloaded > 0);
  ASSERT_TRUE(downloadedChunkCount > 0);
  ASSERT_TRUE(decompressed > 0);
  ASSERT_TRUE(decompressedChunkCount >= downloadedChunkCount);
}
