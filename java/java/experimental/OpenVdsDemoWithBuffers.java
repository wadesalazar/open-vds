...
import org.opengroup.openvds.MemoryVdsGenerator;
import org.opengroup.openvds.QuantizingValueConverter_FloatToByte;
import org.opengroup.openvds.VolumeDataAccessManager;
import org.opengroup.openvds.VolumeDataChannelDescriptor;
import org.opengroup.openvds.VolumeDataLayout;
...
public class OpenVdsDemoWithBuffers {
    ...
    public static void process() throws Exception {
        ...
        for (int y = 0; y < output_height; y++) {
            ...
            for (int x = 0; x < output_width; x++) {
                ...
                posBuffer.put(offset + axis_mapper[0], axis_position + 0.5f);
                posBuffer.put(offset + axis_mapper[1], x_pos);
                posBuffer.put(offset + axis_mapper[2], y_pos);
            }
        }

        System.out.println("Request samples from VolumeDataAccessManager...");
        final FloatBuffer samplesBuffer = BufferUtils.createFloatBuffer(output_width * output_height);

        long request = accessManager.requestVolumeSamples(samplesBuffer, layout, DimensionsND.DIMENSIONS_012, 0, 0,
                posBuffer, elemCount, InterpolationMethod.LINEAR);
        ...
        writeBitmap(outFileName, layout, samplesBuffer, output_width, output_height);
    ...
    static void writeBitmap(String fileName, VolumeDataLayout layout,
                            FloatBuffer data, int output_width, int output_height) throws Exception {
        ...
        for (int y = 0; y < output_height; y++) {
            for (int x = 0; x < output_width; x++) {
                ...
                byte value = converter.convertValue(data.get(inOffset));
